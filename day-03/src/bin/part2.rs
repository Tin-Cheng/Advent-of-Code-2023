...
fn part2(input: &str) -> u32 {
    ...
    let cols: usize = input.lines().next().expect("there should be at least one line").chars().count();
    ...
    fn is_near_symbol(data: &Vec<Vec<char>>, rows: usize, cols: usize, row: usize, col: usize, length: usize) -> bool {
        let min_col = if col > 0 { col - 1 } else { 0 };
        let max_col = if col + length + 1 <= cols { col + length + 1 } else { cols };
    ...
    fn check_nearby_gears(data: &Vec<Vec<char>>, rows: usize, cols: usize, row: usize, col: usize, length: usize) -> Vec<String> {
        ...
        while col < cols {
            if data[row][col].is_digit(10){
                ...
                for gear in &nearby_gears{
                    hash_map.entry(gear.to_string()).or_insert(Vec::new()).push(num.parse::<u32>().unwrap());
                }
                col += length;
            }
            col += 1;
