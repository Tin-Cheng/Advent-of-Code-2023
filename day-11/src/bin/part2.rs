...
fn part2(input: &str, empty_rate: usize) -> usize {
    ...
    for i in 0..galaxies.len() {
        for j in i + 1..galaxies.len() {
            ...
            result += (jx).abs_diff(ix)
                + (jy).abs_diff(iy)
                + empty_rows[jx + 1].abs_diff(empty_rows[ix]) * (empty_rate - 1)
                + empty_cols[jy + 1].abs_diff(empty_cols[iy]) * (empty_rate - 1);
        }
    }
    result
}
...
.......#..
#...#.....";
        assert_eq!(part2(test_case, 2), 374);
        assert_eq!(part2(test_case, 10), 1030);
        assert_eq!(part2(test_case, 100), 8410);
    }
}
